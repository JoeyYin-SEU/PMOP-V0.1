...
AcquisitionWorker::AcquisitionWorker(QWidget* parent, QGraphicsView* displayWindow,
...
void AcquisitionWorker::Start()
{
    ...
    while (m_running)
    {
        try
        {
            ...
            std::chrono::duration<double> usingtime = nowTime - m_begintime;
            tt = usingtime.count()*1000000;
            if (buffer->HasChunks())
            {
                ...
                conversionTime_ms = round(chunkData);
            }
            ...
            auto image = peak::BufferTo<peak::ipl::Image>(buffer).ConvertTo(
                m_pixelFormat, imageBuffer, imageBufferSize, peak::ipl::ConversionMode::HighQuality);
            ...
            emit ImageReceived(qImage, tt, conversionTime_ms);
